...
!*********************************************
    ...
    implicit none
    integer, parameter :: nl =1, N_x = 100, nd =20, nxd = nl*N_x+nd+1
    integer :: i,j,k,l,knt
    integer :: nx,nt
    real :: dx
    real :: pi,eps
...
!        cfl = 3.0
        ...
        write(*,*) '1.Rarfaction, 2.Shock, 3. Burgers Sin, 4. Buckley-Leverett, 5. Non-Convex, 6. linear (sin), 7. variable coeff '
        read(*,*) iexample
        !iexample = 2
...
! mid pts
  ...
  !      umax = maxval(u(1:nx,0))
  !      umin = minval(u(1:nx,0))
  !      if(iexample .eq. 3)then
  !      umax= 1.
  !      umin = 0.
  !      endif
  !      udif = umax-umin
  !      write(*,*) umax, umin,udif
        dt = cfl * dx/1.  ! /udif
        nt = ceiling(T/dt)
...
!***********************************
   ...
   if (t .eq. 0.)then 
    ...
    if (x .le. 0.5 *t ) then
    exact = 2.
    elseif (x .gt. 0.5 *t ) then
    exact = -1.
    endif
    endif
    
    elseif (iexample .eq. 3 .or. iexample == 6 )then
      exact = 1.+2.*sin( x-t )
    ...
    if (iexample .eq. 3 .or. iexample == 6 .or. iexample .eq. 7  .or. iexample .eq. 2) then
		 do i=0, nd
...
!***********************************
     ...
     elseif (iexample .eq. 4) then
          do i=0, nd
		   merged_x(-i)%cell_itg = 1.*dx
           merged_x(merged_cell_total+1+i)%cell_itg = 0.
        enddo
    else
        do i=0, nd
...
!************************************
    ...
    elseif (iexample .eq. 4) then
    do i=0, nd
        element_x_star(-i)%up_itg = 1.* element_x_star(-i)%int_len
        element_x_star(merged_cell_total+1+i)%up_itg = 0.
    enddo
    else
    do i=0, nd
...
!**************************************
    ...
    ! Step 1 ----------------------------------------------------------------------------------------    
         ...
         p => element_x_star(i-2:i+2)
         
		 call flux_comput(p,F_tail_x(i,0)%flux_left,F_tail_x(i,0)%flux_right)  !reconstruction of F_tail at each approx chara line at t^n.
...
! Step 2 ---------------------------------------------------------
        ...
        do i = 1, merged_cell_total
		 ...
		 call flux_comput(p,element_x_star(i)%point_left%flux1,element_x_star(i)%point_right%flux1)  !reconstruction of F_tail at each approx chara line
         
        enddo 
     
        do i = 1, merged_cell_total
        element_x_star(i)%up_itg = us_n(i)-0.25*dt*(element_x_star(i)%point_right%flux1 - element_x_star(i)%point_left%flux1 &
        & + element_x_star(i)%point_right%flux0 - element_x_star(i)%point_left%flux0)
        enddo 
         call bc_element_x_star
         
! Step 3 ---------------------------------------------------------        

        call downstream_locate(dt/2.) 
        do i = 1, merged_cell_total
          !p => element_x_star(i-3:i+3)
        p => element_x_star(i-2:i+2) 
        
		 call flux_comput(p,element_x_star(i)%point_left%flux2,element_x_star(i)%point_right%flux2)  !reconstruction of F_tail at each approx chara line
         
        enddo 
     
        do i = 1, merged_cell_total
        element_x_star(i)%up_itg = us_n(i)-dt/6.*(element_x_star(i)%point_right%flux1 - element_x_star(i)%point_left%flux1 &
        & + element_x_star(i)%point_right%flux0 - element_x_star(i)%point_left%flux0 &
        & + 4.*(element_x_star(i)%point_right%flux2 - element_x_star(i)%point_left%flux2 ))
        enddo 
...
!F_{j-1/2}  
    ...
    call lambda_local( fleft_minus,fleft_plus, p(3)%point_left%char, p(3)%point_left%coor,lambda  )
    
    fleft = 0.5*(f(fleft_minus,p(3)%point_left%coor) - p(3)%point_left%char *fleft_minus &
        & + f(fleft_plus,p(3)%point_left%coor)-p(3)%point_left%char *fleft_plus) - 0.5*lambda*(fleft_plus-fleft_minus )
    ...
    call lambda_local( fright_minus,fright_plus, p(3)%point_right%char, p(3)%point_right%coor,lambda)

    fright = 0.5*(f(fright_minus,p(3)%point_right%coor)-p(3)%point_right%char *fright_minus &
         & + f(fright_plus,p(3)%point_right%coor)-p(3)%point_right%char*fright_plus)-0.5*lambda*(fright_plus-fright_minus)
    
    !     write(*,*) fright_minus,fright_plus, lambda,fright
...
!    gc = (1.-gamma)*gamma
  ...
  !  al = 2.*(-len3*avg_l*len3+lenl*avg_3*len3)/(len3*(lenl+len3))
    al = 2.*(-len3*avg_l*len3+lenl*avg_3*len3)/(lenl*(lenl+len3))
    bl = avg_3
    f_l = al*xi+bl
    
!    ar = 2.*(-lenr*avg_3*len3+len3*avg_r*len3)/(len3*(lenr+len3))
    ar = 2.*(-lenr*avg_3*len3+len3*avg_r*len3)/(lenr*(lenr+len3))
    br = avg_3
    f_r = ar*xi+br
...
!***************************************************************************
    ...
    ptc = (ptl+ptr)/2.
    if((ptl-(1.+ 2.*sin(xx-ptl*tt))) * (ptc-(1.+2.*sin(xx-ptc*tt))) .lt. 0. )then
       ptr=ptc
    elseif((ptr-(1.+ 2.*sin(xx-ptr*tt))) * (ptc-(1.+2.*sin(xx-ptc*tt))) .lt. 0. )then
       ptl=ptc
    elseif((ptc-(1.+2.*sin(xx-ptc*tt))) .eq. 0. )then
    burgex = ptc
    return
    else
   ...
   !  do while(abs(q1-sin(xx-q1*tt)) > eps )
        q0 = qtemp
        q1 = q0 - ( q0-(1.+2.*sin(xx-q0*tt)) )/ ( 1. + 2.* tt*cos( xx-q0*tt ))
        qtemp = q1
    enddo
    burgex = q1
