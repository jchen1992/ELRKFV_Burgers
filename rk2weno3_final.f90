...
!********************************************
		...
		dx = (xright-xleft)/nx
 		if(iexample .eq. 2 .or. iexample .eq. 1)then
...
! Step 2 ---------------------------------------------------------
        ...
        do i = 1, merged_cell_total
        element_x_star(i)%up_itg = us_n(i)-0.5*dt*(element_x_star(i)%point_right%flux1 - element_x_star(i)%point_left%flux1 &
 ...
 enddo
   ...
   !  do while(abs(q1-sin(xx-q1*tt)) > eps )
    ...
    integer :: l  
		 ...
		 enddo
		 write(*,*) 'T=',T,'tt=',tt,'nt=',nt,'dt=',dt,'dx=',dx
         pause
